"use strict";
var tinytim = require('tinytim'), dateFormat = require('dateformat'), utils = require('./utils'), path = require('path'), settings = require('./settings').settings, debug = require('debug'), memoize = require('lodash/memoize');
...
function _getDebugFn(path, prefix) {
	var debugNamespace = path.replace(prefix, '').split('/').join(':').replace(/^:/, '').replace(/\), <anonymous>$/, ':anonymous');
	return debug(debugNamespace);
}

var getDebugFn = memoize(_getDebugFn);
...
module.exports = (function() {
	...
	var _config = {
		...
		transport : function(data) {
			var debugMsg = "line " + data.line + ":" + data.pos + " " + data.message;
			var debugFn = getDebugFn(data.path, data.prefix);
			if (debugFn.enabled) {
				debugFn(debugMsg)
			} else {
